#ifndef VVLODASYNCGLOBJECT_H
#define VVLODASYNCGLOBJECT_H
...
/**
 * @brief The vvLODAsyncGLObject class manages scene objects with multiple
 ...
 *
 * Rather than storing vvGLObject state in member variables, it should be
 * stored in a subclass of vvLODAsyncGLObject::ObjectState, which is passed
 ...
 * objectState() method.
 */
class vvLODAsyncGLObject : public vvGLObject
{
public:
  ...
  /**
   * Subclass this to store internal data for this vvGLObject. Parameters
   * should be stored here so that they can be passed to the LOD pipelines
   * safely. There should only be one per vvLODAsyncGLObject subclass. It is
  ...
  struct ObjectState
  {
     ...
     * vvInteractor, etc.
     */
    virtual void update(const vvApplicationState &state) = 0;
  };
  ...
  struct DataPipeline
  {
    ...
    virtual void configure(const ObjectState &objState,
                           const vvApplicationState &appState) = 0;
...
public:
  ...
  void init(const vvApplicationState &appState) override;

  void initVvContext(vvContextState &contextState,
                     GLContextData &contextData) const final;

  void syncApplicationState(const vvApplicationState &state) final;
...
protected:
  ...
  /**
   * Return the object state for this vvLODAsyncGLObject. The template parameter
...
private: // vtkLODAsyncGLObject virtual API:
   ...
   * used to hold this vvLODAsyncGLObject's state.
   */
...
private: // Private nested classes, implementation  details, etc:
  ...
  /**
   * The DataItem for vvLODAsyncGLObject. This should NOT be subclassed by
  ...
  struct DataPipelineManager
  {
    ...
    vvProgressCookie *cookie{nullptr};
  };
...
struct vvLODAsyncGLObject::LODIteratorImpl
{
...
//------------------------------------------------------------------------------
inline vvLODAsyncGLObject::iterator
vvLODAsyncGLObject::bestToFast(bool skipInvalid)
{
...
//------------------------------------------------------------------------------
inline vvLODAsyncGLObject::const_iterator
vvLODAsyncGLObject::bestToFast(bool skipInvalid) const
{
...
//------------------------------------------------------------------------------
inline vvLODAsyncGLObject::iterator
vvLODAsyncGLObject::fastToBest(bool skipInvalid)
{
...
//------------------------------------------------------------------------------
inline vvLODAsyncGLObject::const_iterator
vvLODAsyncGLObject::fastToBest(bool skipInvalid) const
{
  return const_iterator(const_iterator::Direction::FastToBest, m_dataPipelines,
                        skipInvalid);
}

#endif // VVLODASYNCGLOBJECT_H
